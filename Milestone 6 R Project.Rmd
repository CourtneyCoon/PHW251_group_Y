...
### *Figure 1:*
Here we have identified high priority counties based on demographic data only
with priority given to counties with: (1) renter ratios above the median of all
...
quantile for all counties (1st quantile = 25.9 people per square mile; 3rd 
quantile = 333.5 people per square mile). Each dot represents a county in 
California. Counties that are named on the figure are those meeting all of the
high priority demographic criteria: Mendicino, San Luis Obispo, Sonoma, San
Mateo, Butte, Humboldt, and Mono Counties.
...
and lower population density", 
  ...
  scale_color_manual(labels = c("Low priority (>333)", "Medium priority (26-333)",
        "<26"), values = c("#b2e2e2", "#66c2a4", "#238b45"))
...
merged_data_for_table <- merged_data[, table_col_order]
 table<-merged_data_for_table%>%
  ...
  mutate(high_four_pts= sum(c_across(7:9) == "High priority", na.rm = TRUE),
         medium_two_pts= sum(c_across(7:11) == "Medium priority", na.rm = TRUE),
